    ...
    def update(self, instance: ACLBase, validated_data: dict[str, Any]) -> ACLBase:
        ...
        permissions: dict[str, HistoricalPermission] = {
            "full": cast(
                ...
                HistoricalPermission.objects.get(codename=f"{instance.id}.{ACLType.Full.id}"),
            ),
            "writable": cast(
                "HistoricalPermission",
                HistoricalPermission.objects.get(codename=f"{instance.id}.{ACLType.Writable.id}"),
            ),
            "readable": cast(
                "HistoricalPermission",
                HistoricalPermission.objects.get(codename=f"{instance.id}.{ACLType.Readable.id}"),
            ),
        }
