from typing import cast

from rest_framework import status
from rest_framework.request import Request
from rest_framework.response import Response
from rest_framework.views import APIView

from role.models import Role
from user.models import User


class RoleAPI(APIView):
    def delete(self, request: Request, role_id: int, format: str | None = None) -> Response:
        try:
            role = Role.objects.get(pk=role_id)
        except Role.DoesNotExist:
            return Response(f"Role not found(id:{role_id})", status=status.HTTP_404_NOT_FOUND)

        user = cast("User", request.user)
        if not role.is_editable(user):
            return Response(
                f"Permission error to delete the Role({role.name})",
                status=status.HTTP_401_UNAUTHORIZED,
            )

        # This deletes target Role instance.
        # Just before doing it, this copies name for response message.
        role_name = role.name
        role.delete()
        return Response(
            f"Succeeded in deleting Role({role_name})",
            status=status.HTTP_204_NO_CONTENT,
        )
