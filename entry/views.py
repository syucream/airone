...
def _validate_input(recv_data: dict[str, Any], obj: Entry | Entity) -> HttpResponse | None:
    ...
    for attr_data in recv_data["attrs"]:
        # The validation checks below run against EntityAttr (the schema definition),
        # regardless of whether the incoming object is a live Entry (its Attributes'
        # schema field) or the Entity itself.
        attr: EntityAttr | None = None
        if isinstance(obj, Entry):
            entry_attr: Attribute | None = None
            if attr_data["id"]:
                entry_attr = obj.attrs.filter(id=attr_data["id"]).first()

            if entry_attr is not None:
                attr = entry_attr.schema
...
def index(request: HttpRequest, entity_id: int) -> HttpResponse:
    ...
    entity, error = get_obj_with_check_perm(user, Entity, entity_id, ACLType.Readable)
    if entity is None:
    ...
    if custom_view.is_custom("list_entry", entity.name):
        # list custom view
        rendered: HttpResponse = custom_view.call_custom(
...
def create(request: HttpRequest, entity_id: int) -> HttpResponse:
    ...
    context = {
        ...
        # django-stubs sees Django's auth Group.objects here rather than our subclass's
        # custom manager, so it fails to resolve the added is_active field.
    ...
    if custom_view.is_custom("create_entry", entity.name):
        # show custom view
        rendered = custom_view.call_custom(
...
def edit(request: HttpRequest, entry_id: int) -> HttpResponse:
    user = cast(User, request.user)
    entry, error = get_obj_with_check_perm(user, Entry, entry_id, ACLType.Writable)
    if entry is None:
        assert error is not None
        return error
...
def show(request: HttpRequest, entry_id: int) -> HttpResponse:
    user = cast(User, request.user)
    entry, error = get_obj_with_check_perm(user, Entry, entry_id, ACLType.Readable)
    if entry is None:
        assert error is not None
        return error
...
def revert_attrv(request: HttpRequest, recv_data: dict[str, Any]) -> HttpResponse:
    ...
    assert latest_value is not None, "an attribute always has at least one AttributeValue"
